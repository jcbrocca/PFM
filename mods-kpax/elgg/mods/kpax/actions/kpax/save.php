...
//jcbrocca - ccamera
$platform = get_input('plataformes');
$skill = get_input('skills');
...
//jcbrocca - ccamera
$kPAXgame->idPlatform = $platform; 
$kPAXgame->idSkill = $skill; 
...
if($objKpax->addGame(
    ...
    register_error(elgg_echo('kpax:save:failed:service_1'));
}
else
{
    if($objKpax->addDelTagsGame(
          $_SESSION["campusSession"], 
          $kPAXgame->getGUID(), 
          get_input('tags')
        )!="OK"){
     register_error(elgg_echo('kpax:save:failed:service_2'));
    }

    if($objKpax->addDetailGame(
         $_SESSION["campusSession"], 
         $kPAXgame->getGUID(), 
         $kPAXgame->description, 
         $kPAXgame->logo, 
         $kPAXgame->banner, 
         $kPAXgame->videourl
       )!="OK"){
        register_error(elgg_echo('kpax:save:failed:service_3'));
    }

    if($objKpax->addImagesGame(
        $_SESSION["campusSession"], 
        $kPAXgame->getGUID(), 
        get_input('images')
       )!="OK"){
     register_error(elgg_echo('kpax:save:failed:service_4'));
    }
}

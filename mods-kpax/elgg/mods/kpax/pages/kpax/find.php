...
//----------------
//AUTHOR: vcorralb Modify by jcbrocca
//----------------
	...
	$content.="<div class='listado_juegos'>";
		$content.="<div class='menu_vertical'>";
			$content.="<h1>".elgg_echo('kpax:game:category')."</h1>";
				...
				$content .= "<form method='post' action='find'>";
				$content .= "<fieldset>";
...
$gameList = $objKpax->getListGamesSearch(
		$name, 
		$category, 
		$platform, 
		$ski, 
		...
		$offset, 
		$limit, 
		$_SESSION["campusSession"]);


// Obtener datos de GameDetail
...
if(isset($gameList->games) && sizeof($gameList->games) > 0) {
   $content .= elgg_view( 
		'kpax/find_games_list',
		array(
			'objGameList' => $gameList->games, 
    ...
    $content .= '<br/><div><p>' . elgg_echo('kpax:nogames') . '</p></div>';
}
...
//Pagination
			...
			if ($gameList->offset > 0){
				...
				$content .= "<input type='submit'  class='elgg-button elgg-button-action' value='".elgg_echo('kpax:game:previous')."' />";
				$content .= "</fieldset>";
				$content .= "</form>";
				$content .= "</div>";
			}
			if ($gameList->offset + $gameList->limit < $gameList->total->integer){
				...
				$content .= "<input type='hidden' name='sort' value='".$sort."'/>";
				$content .= "<input type='hidden' name='offset' value='".($offset+$limit)."'/>";
				$content .= "<input type='submit' class='elgg-button elgg-button-action' value='".elgg_echo('kpax:game:next')."' />";
				$content .= "</fieldset>";
				$content .= "</form>";
				$content .= "</div>";

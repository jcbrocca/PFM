...
$basic_features=list_basic_features();

$categories=$basic_features[0];
foreach($categories as $cat) {$cats[$cat->idCategory] = $cat->name ;}

$platforms=$basic_features[1];
foreach($platforms as $plat) {$plats[$plat->idPlatform] = $plat->name ;}

$skills=$basic_features[2];
foreach($skills as $skl) {$skls[$skl->idSkill] = $skl->name ;}    


$category = elgg_extract('category', $vars, ''); //NOU
$creationDate = elgg_extract('creationDate', $vars, ''); //NOU

$skill = elgg_extract('skills', $vars, '');
$platform = elgg_extract('plataformes', $vars, '');
...
if ($objGameDet != null){
  ...
  $category = $objGame->idCategory;
  $platform = $objGame->idPlatform;
  $skill = $objGame->idSkill;
...
<div>
    ...
    <?php 
        echo elgg_view('input/dropdown', array('name' => "category",  'value' => $category,  'options_values' => $cats)); 
    ?>
...
<div> 
    ...
    <?php 
        echo elgg_view('input/dropdown', array('name' => "plataformes", 'value' => $platform,  'options_values' => $plats)); 
    ?>
</div>    
...
<div>
    ...
    <?php 
        echo elgg_view('input/dropdown', array('name' => "skills",  'value' => $skill, 'options_values' => $skls)); 
    ?>
</div>
